...
**SMWS Japan** のブラウジング体験を向上させる Chrome 拡張機能です。  
数字だけのコード（例：`41.190`）に蒸留所名を自動表示し、コードの絞り込み検索も使いやすくします。

---

## 機能

- **蒸留所名の自動表示** — 商品一覧・商品詳細・お気に入り・マイページ・注文詳細のコードに蒸留所名をバッジ表示
- **検索ボックスの改善** — 蒸留所コードの絞り込みで蒸留所名が候補として表示される
- **クリアボタン** — 蒸留所コード入力欄をワンクリックでリセット
...
5. 「パッケージ化されていない拡張機能を読み込む」から解凍したフォルダを選択
